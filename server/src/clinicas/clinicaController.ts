...
export const criarClinica = async (req: Request, res: Response): Promise<void> => {
  ...
  if (!nome || !email || !senha || !endereco) {
    res.status(400).json({ message: 'Campos obrigatórios: nome, email, senha, endereco' })
    return
  }

  if (!endereco.cep || !endereco.rua || !endereco.estado || !endereco.numero || !endereco.complemento) {
    res.status(400).json({ message: 'Campos obrigatórios no endereço: cep, rua, estado, numero, complemento' })
    return
  }
